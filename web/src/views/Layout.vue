  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <Transition name="slide">
      <aside v-show="sidebarOpen || isDesktop"
             class="fixed lg:relative z-50 lg:z-auto w-72 lg:w-64 flex-shrink-0 flex flex-col h-full"
        ...
        <div class="relative z-10 px-5 pt-6 pb-4 flex items-center gap-3">
          ...
          <button @click="sidebarOpen = false"
                  class="lg:hidden absolute top-4 right-4 text-white/50 hover:text-white p-1.5 rounded-lg hover:bg-white/10 transition-all">
            <X :size="18" />
          </button>
          ...
          <div class="text-white font-bold text-lg leading-none self-center">Vane</div>
        </div>
    ...
    <main class="flex-1 overflow-y-auto min-w-0">
      <header class="sticky top-0 z-30 bg-white/80 backdrop-blur-sm border-b border-slate-100 px-4 sm:px-8 py-4 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-0">
          <!-- Mobile hamburger -->
          <button @click="sidebarOpen = true"
                  class="lg:hidden flex-shrink-0 p-2 rounded-xl text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-all">
            <Menu :size="20" />
          </button>
          <div class="min-w-0">
            <h1 class="text-base sm:text-lg font-semibold text-slate-900 truncate">{{ currentPage.label }}</h1>
            <p class="text-xs text-slate-400 hidden sm:block">{{ currentPage.desc }}</p>
          </div>
        </div>
        <div class="flex items-center gap-2 flex-shrink-0">
          <!-- Lang switch -->
          <button @click="i18n.toggle()"
                  class="text-xs text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 px-2.5 py-1.5 rounded-full border border-slate-200 transition-all hidden sm:block">
            {{ i18n.t('switchLang') }}
          </button>
          <div class="flex items-center gap-1.5 text-xs text-emerald-600 bg-emerald-50 px-2.5 py-1.5 rounded-full border border-emerald-200">
            <span class="status-dot active"></span>
            <span class="hidden sm:inline">{{ i18n.t('running') }}</span>
          </div>
          <div class="text-xs text-slate-400 font-mono bg-slate-50 px-2.5 py-1.5 rounded-full border border-slate-100 hidden md:block">
