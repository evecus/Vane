  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 flex-shrink-0 flex flex-col h-full relative"
      ...
      <nav class="relative z-10 flex-1 px-3 py-2 space-y-1 overflow-y-auto">
        <div class="px-3 py-2">
          <span class="text-white/30 text-xs font-bold uppercase tracking-widest">Main</span>
        </div>
        <router-link v-for="item in navItems" :key="item.to" :to="item.to"
          ...
          <span v-if="item.badge" class="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full">
            {{ item.badge }}
          </span>
        </router-link>

        <div class="px-3 py-2 mt-4">
          <span class="text-white/30 text-xs font-bold uppercase tracking-widest">System</span>
        </div>
      ...
      <div class="relative z-10 p-4 border-t border-white/10">
        <div class="flex items-center gap-3">
          ...
          <button @click="auth.logout()" class="text-white/40 hover:text-white/70 transition-colors p-1.5 rounded-lg hover:bg-white/10">
            <LogOut :size="15" />
          </button>
        </div>
      </div>
...
const navItems = [
  {
    ...
    desc: 'Overview of all services',
  },
  {
    to: '/portforward', label: '端口转发', icon: ArrowLeftRight,
    gradient: 'linear-gradient(135deg, #3b82f6, #06b6d4)',
    shadow: '0 4px 15px rgba(59,130,246,0.4)',
    desc: 'TCP/UDP port forwarding rules',
  },
  {
    to: '/ddns', label: '动态域名 DDNS', icon: Globe,
    gradient: 'linear-gradient(135deg, #10b981, #059669)',
    shadow: '0 4px 15px rgba(16,185,129,0.4)',
    desc: 'Dynamic DNS updates',
  },
  {
    to: '/webservice', label: 'Web 服务', icon: Server,
    gradient: 'linear-gradient(135deg, #8b5cf6, #ec4899)',
    shadow: '0 4px 15px rgba(139,92,246,0.4)',
    desc: 'Reverse proxy & HTTPS',
  },
  {
    to: '/tls', label: 'TLS 证书', icon: Shield,
...
const currentPage = computed(() => {
  const found = navItems.find(n => n.to === route.path) || { label: 'Settings', desc: 'System configuration' }
  return found
})
