  ...
  <div class="animate-fade-in">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
      ...
      <section class="glass-card overflow-hidden order-4 xl:order-none">
        ...
        <div class="px-5 py-2">
          <!-- 第一行：程序版本 + 项目地址 -->
          <div class="grid grid-cols-2 gap-x-4 items-center py-3 border-b border-slate-100">
            <div>
              <div class="text-xs text-slate-400 mb-0.5">程序版本</div>
              <div class="font-mono text-sm font-semibold text-slate-700">{{ form.version || 'dev' }}</div>
            </div>
            <div>
              <div class="text-xs text-slate-400 mb-1">项目地址</div>
              <a href="https://github.com/evecus/Vane" target="_blank" rel="noopener noreferrer"
                 class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-white text-xs font-medium transition-all active:scale-95 shadow-sm">
                <svg viewBox="0 0 24 24" class="w-3.5 h-3.5 flex-shrink-0 fill-current" xmlns="http://www.w3.org/2000/svg">
          ...
          <!-- 第二行：数据加密 + 密码哈希 -->
          <div class="grid grid-cols-2 gap-x-4 items-center py-3">
            <div>
              <div class="text-xs text-slate-400 mb-1">{{ i18n.t('encryption') }}</div>
              <span class="font-mono text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">AES-256-GCM</span>
            </div>
            <div>
              <div class="text-xs text-slate-400 mb-1">{{ i18n.t('passwordHash') }}</div>
