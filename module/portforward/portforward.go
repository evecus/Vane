// Package portforward provides TCP/UDP/dual-stack port forwarding.
// When protocol is "both", two separate listeners (TCP + UDP) are started.
package portforward
...
type Manager struct {
	cfg     *config.Config
	mu      sync.Mutex
	workers map[string][]*worker // one rule may have 2 workers (tcp+udp)
...
func (m *Manager) Start(id string) error {
	...
	var workers []*worker
	protos := protocolList(rule.Protocol)
	for _, proto := range protos {
		w := newWorker(*rule, proto, st)
		workers = append(workers, w)
		go w.run()
	}
	m.workers[id] = workers
	return nil
}
...
// protocolList returns the actual protocol strings to listen on.
// "both" expands to ["tcp", "udp"].
func protocolList(proto string) []string {
	switch proto {
	case "both":
...
func (w *worker) runTCP(addr string) {
...
func (w *worker) runUDP(addr string) {
	...
	for {
		...
		go w.forwardUDP(conn, caddr, raddr, buf[:n])
	}
}

func (w *worker) forwardUDP(conn *net.UDPConn, caddr, raddr *net.UDPAddr, data []byte) {
	...
	_, _ = conn.WriteToUDP(resp[:n], caddr)
}
