...
// IssueCert requests a new certificate via ACME DNS-01 challenge.
func (m *Manager) IssueCert(certID string) error {
	...
	if cert.Email == "" {
		return fmt.Errorf("cert %s: email is required for ACME registration", certID)
	}
	...
	cfg.CADirURL = lego.LEDirectoryProduction // ← use production Let's Encrypt
