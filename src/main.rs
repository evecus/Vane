...
pub static EMBEDDED_FILES: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/web/dist");

#[derive(Parser, Debug)]
struct Cli {
    #[arg(long)]
    config: Option<PathBuf>,
    /// Disable features, comma-separated (e.g. "systeminfo")
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("\n  ✨ Dashboard : http://0.0.0.0:{port}{entry_path}\n");
    ...
    let app = Router::new()
        ...
        .route("/api/admin/logs", get(get_admin_logs).post(append_admin_log))
        ...
        // Port check (GET with query param, Go-compatible)
        .route("/api/check-port", get(check_port_query).post(check_port))
        // Port Forward
        .route("/api/portforward", get(list_port_forwards).post(create_port_forward))
        .route("/api/portforward/:id", put(update_port_forward).delete(delete_port_forward))
        ...
        // Web Service
        .route("/api/webservice", get(list_webservices).post(create_webservice))
        ...
        .route("/api/webservice/:id", put(update_webservice).delete(delete_webservice))
        .route("/api/webservice/:id/toggle", post(toggle_webservice))
        .route("/api/webservice/:id/routes", get(list_routes).post(create_route))
        .route("/api/webservice/:id/routes/:rid", put(update_route).delete(delete_route))
        ...
        .route("/api/ipfilter/:id", put(update_ipfilter).delete(delete_ipfilter))
