...
pub async fn update(
...

    {
        ...
        if let Some(r) = cfg.port_forwards.iter_mut().find(|r| r.id == id) {
            ...
            if let Some(v) = req.target_ip {
                r.target_ip = v;
            }
            if let Some(v) = req.target_port {
                r.target_port = v;
            }
...
pub async fn delete_pf(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    state.pf.stop(&id);
    ...
    let modified_rules = state
        .cfg
        .clean_scopes_for_deleted_target("portforward", &id);
...
pub async fn toggle(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    if let Some(rule) = rule {
        let dd = state.cfg.read().data_dir.clone();
        if let Some(dd) = dd {
            let _ = db::save_port_forward(&dd, &rule);
        }
    }
...
pub async fn stats(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
