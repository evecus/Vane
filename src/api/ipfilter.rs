...
pub async fn list_targets(State(state): State<AppState>) -> impl IntoResponse {
    ...
    for pf in &cfg.port_forwards {
        targets.push(
            serde_json::json!({"type": "portforward", "target_id": pf.id, "target_name": pf.name}),
        );
    }
...
pub async fn create_rule(
    ...
    if let Some(desc) = conflict {
        return (
            StatusCode::CONFLICT,
            Json(serde_json::json!({"error": format!("scope conflict: {}", desc)})),
        )
            .into_response();
    }
    ...
    if let Some(bad) = validate_ips(&manual_ips) {
        return (
            ...
            Json(serde_json::json!({"error": format!("invalid IP/CIDR: {}", bad)})),
        )
            .into_response();
    }
